...
class ReferrerSeeder extends Seeder
{
    ...
    {
        // Ambil contoh region_id dan region_code dari database
        $regionCabangKalbar = DB::table('regions')->where('code', 'KB00')->first(); // Asumsi kode 'KB00' ada dari RegionSeeder
        $regionUnitPontianak = DB::table('regions')->where('code', 'PNK01')->first(); // Asumsi kode 'PNK01' ada

        $referrers = [];

        if ($regionCabangKalbar) {
            $referrers[] = [
                'name' => 'Budi Marketing Cabang',
                'type' => 'MARKETING',
        ...
        if ($regionUnitPontianak) {
            $referrers[] = [
                'name' => 'Ormas Maju Bersama Pontianak',
                'type' => 'ORMAS',
            ...
            $referrers[] = [
                'name' => 'Citra Marketing Pontianak',
                'type' => 'MARKETING',
                'region_id' => $regionUnitPontianak->id,
