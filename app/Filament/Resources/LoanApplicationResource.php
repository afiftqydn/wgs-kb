...
use Filament\Forms\Components\FileUpload; // MODIFIKASI: Import yang diperlukan
                ...
                Wizard::make([
                    Wizard\Step::make('Informasi Permohonan')
                        ->schema([
                            ...
                            // BARU: Input Jenis Usaha Nasabah
                            TextInput::make('customer_business_type')
                                ->label('Jenis Usaha Nasabah')
                                ->required()
                                ->columnSpanFull(),
                            ...
                            Select::make('input_region_id')
                                ...
                                ->disabled(fn () => !auth()->user()->hasAnyRole(['Tim IT', 'Kepala Cabang', 'Kepala Unit', 'Analis Unit', 'Admin Unit']))
                                ->required(),
                    ...
                    Wizard\Step::make('Unggah Dokumen Pendukung')
                        ->schema([
                            // MODIFIKASI: Repeater disederhanakan menjadi satu unggahan file saja
                             Forms\Components\Repeater::make('documents')
                                ->relationship()
                                ->label('Unggah Berkas')
                                ->schema([
                                    Forms\Components\Hidden::make('document_type')
                                        ->default('Berkas Lampiran'), // Otomatis set jenis dokumen
                                ...
                                ->columnSpanFull()
                                ->collapsible()
                                ->minItems(1) // Hanya bisa 1 item
                                ->maxItems(1) // Hanya bisa 1 item
                                ->addable(false) // Tombol tambah dihilangkan
                                ->deletable(false) // Tombol hapus dihilangkan
                                ->reorderable(false), // Tidak bisa diurutkan
                        ]),
                ])->columnSpanFull(),
            ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                // BARU: Menambahkan kolom Jenis Usaha
                Tables\Columns\TextColumn::make('customer_business_type')->label('Jenis Usaha')->searchable()->sortable()->toggleable(isToggledHiddenByDefault: true),
                ...
                // BARU: Menambahkan kolom Status Pembayaran dengan badge
                Tables\Columns\TextColumn::make('payment_status')->label('Pembayaran')->badge()
                    ->color(fn (string $state): string => match ($state) {
                        'Belum Transfer' => 'warning',
                        'Sudah Transfer' => 'success',
                        default => 'gray',
                    })->searchable()->sortable(),
            ...
            ->filters([
                ...
                // BARU: Filter untuk status pembayaran
